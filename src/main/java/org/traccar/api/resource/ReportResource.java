...
import org.traccar.reports.Chart;
import org.traccar.reports.Dds;
...
public class ReportResource extends BaseResource {
    ...
    @Path("events/{id}")
    @PUT
    public Response update(Event entity) throws SQLException {
     //   LOGGER.info("updatee for event");

///   Context.getPermissionsManager().checkAdmin(getUserId());
        Context.getPermissionsManager().updateEvent(entity);
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public Collection<ChartReport> getChart(
            @QueryParam("deviceId") final List<Long> deviceIds, @QueryParam("groupId") final List<Long> groupIds,
            @QueryParam("from") Date from, @QueryParam("to") Date to) throws SQLException {
        LogAction.logReport(getUserId(), "Chart", from, to, deviceIds, groupIds);
        return Chart.getObjects(getUserId(), deviceIds, groupIds, from, to);
    }
    
    @Path("dds")
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public Collection<DdsReport> getDds(
            @QueryParam("deviceId") final List<Long> deviceIds, @QueryParam("groupId") final List<Long> groupIds,
            @QueryParam("from") Date from, @QueryParam("to") Date to) throws SQLException {
        LogAction.logReport(getUserId(), "Dds", from, to, deviceIds, groupIds);
        return Dds.getObjects(getUserId(), deviceIds, groupIds, from, to);
    }
}
