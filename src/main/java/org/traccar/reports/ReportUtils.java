...
/*else if (fuelCase>1){
    BigDecimal value = BigDecimal.valueOf( (firstPosition.getDouble(Position.KEY_FUEL_LEVEL)/10)
    - (int)(lastPosition.getDouble(Position.KEY_FUEL_LEVEL)/10));
return value.setScale(1, RoundingMode.HALF_EVEN).doubleValue();
}
*/
...
///trip function
    private static TripReport calculateTrip(
        ...
        if(positions.get(i+1) != null)
{    

    

    if (Double.compare(positions.get(i+1).getDouble(Position.KEY_FUEL_LEVEL), positions.get(i).getDouble(Position.KEY_FUEL_LEVEL)) < 0) 
{

  //  spentfuel+=positions.get(i).getDouble(Position.KEY_FUEL_LEVEL)-positions.get(i+1).getDouble(Position.KEY_FUEL_LEVEL);
  spentfuel+=ReportUtils.calculateFuel(positions.get(i), positions.get(i+1));
